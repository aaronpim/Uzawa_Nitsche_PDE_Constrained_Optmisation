...
def geo_train(target, para, x, y, learning_rate = 1e-3,  device = torch.device("cpu")):       
        ...
        for epoch in range(para[2]*para[3]):
                ...
                if epoch % para[2] == 0:
                        ...
                        z = z + para[1] * (Au - f)
                        grey_plot(u,x,y,f'./Vary_alpha_{logalpha}_nPts_{npts}_eps_0.05x{epstra}',f'State_frame_{epoch}')
                        grey_plot(f,x,y,f'./Vary_alpha_{logalpha}_nPts_{npts}_eps_0.05x{epstra}',f'Cntrl_frame_{epoch}')
        toc = time.time()
        elapseTime = toc - tic
...
for epstra in eps_values:
        alpha = 10**(-logalpha)
        para = para_def(alpha, 0.25*alpha, EpUp = 50, NoUp = 50, h_n = 40, printyn = True, epsilon = 0.05*epstra)
        
        path = f'./Vary_alpha_{logalpha}_nPts_{npts}_eps_0.05x{epstra}'
