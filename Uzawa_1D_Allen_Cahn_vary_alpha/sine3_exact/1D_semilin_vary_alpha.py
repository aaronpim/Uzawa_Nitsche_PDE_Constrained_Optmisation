...
def plot_line(x, u, uexact, f, fexact, path,filename):
        ...
        plt.savefig(f"{path}/{filename}.pdf", format='pdf')
        plt.close()
def plot_gif_frame(x, u, uexact, path, filename, c, la):
        plt.figure()
        plt.plot(x.detach().numpy(), uexact.detach().numpy(), '--', color=c, label=la[0], alpha=0.5)  
        plt.plot(x.detach().numpy(), u.detach().numpy(), 'o', color=c, label=la[1], alpha=0.5)  
        plt.grid(True)  # Turns on the grid
        plt.legend(loc='upper right')
        plt.savefig(f"{path}/{filename}.pdf", format='pdf')
        plt.close() 
...
def geo_train(target, para, x, exact = torch.tensor([float('nan')]), learning_rate = 1e-3,  device = torch.device("cpu")):       
        ...
        def generatorCriterion(x, z, target, para):
                ...
                Au = -((1/para[6])**2)*u*(1-u**2)-uxx
                
                # Second term: alpha/2 * Norm{f}^2
                term2_1 = para[0] / 4 * dx * Int1D((Au)**2)
        ...
        for epoch in range(para[2]*para[3]):
                ...
                if epoch % para[2] == 0:
                        ...
                        z[0] = 0
                        z[-1]= 0
                        #plot_gif_frame(x, u, exact[0], f'./Vary_alpha_{para[0]}_eps_{para[6]}',f'state_frame_{epoch}', 'orange', ['$u_\\theta$','$uexact$'])
                        #plot_gif_frame(x, f, exact[1], f'./Vary_alpha_{para[0]}_eps_{para[6]}',f'control_frame_{epoch}', 'red', ['$f_\\theta$','$fexact$'])
                        #plot_gif_frame(x, z, 0.5*para[0]*exact[1], f'./Vary_alpha_{para[0]}_eps_{para[6]}',f'adjoint_frame_{epoch}', 'blue', ['$z_\\theta$','$zexact$'])
        toc = time.time()
        elapseTime = toc - tic
        u, f = generator_NN(x)              
...
npts = 201
repeats = 1
logalpha_values = range(0,8)
eps_value = 1
for logalpha in logalpha_values:
        alpha = 10**(-logalpha)
        para = para_def(alpha, 0.25*alpha, EpUp = 10, NoUp = 500, h_n = 40, printyn = True, epsilon = 0.1*eps_value)
        
        path = f'./Vary_alpha_{para[0]}_eps_{para[6]}'
        if not os.path.isdir(path):
                os.makedirs(path)
        x = Domains(npts)
        ##############################################################################################
        alpha = 10**(-logalpha)
        exact_u   = torch.sin(-3*pi*x)
        ...
        exact_z   = alpha * exact_f
        exact_zx  = torch.autograd.grad(exact_z,  x, grad_outputs=torch.ones_like(x), create_graph=True, only_inputs=True)[0]
        exact_zxx = torch.autograd.grad(exact_zx, x, grad_outputs=torch.ones_like(x), create_graph=True, only_inputs=True)[0]
        target    = exact_u -exact_zxx - ((1/para[6])**2)*exact_z*(1- 3*exact_u**2)
        ##############################################################################################
        u = 0
        f = 0
        err_history  = 0
        ...
        for i in range(0,repeats):
                gNN, ui, fi, err_historyi, loss_historyi, elapseTime = geo_train(target, para, x, exact = [exact_u,exact_f], learning_rate = 1e-3) 
                u += ui/repeats
                f += fi/repeats
        ...
        mysave(torch.abs(u-exact_u), path, f'Pointwise_State')
        mysave(torch.abs(f-exact_f), path, f'Pointwise_Control')
