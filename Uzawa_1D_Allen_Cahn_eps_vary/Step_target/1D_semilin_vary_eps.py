...
def geo_train(target, para, x, exact = torch.tensor([float('nan')]), learning_rate = 1e-3,  device = torch.device("cpu")):       
        ...
        for epoch in range(para[2]*para[3]):
                ...
                if epoch % para[2] == 0:
                        ...
                        z[0] = 0
                        z[-1]= 0
                        #plot_gif_frame(x, u, target, f'./Vary_alpha_{para[0]}_eps_{para[6]}',f'state_frame_{epoch}', 'orange', ['$D target$','$u_\\theta$'])
                        #plot_gif_frame(x, f, exact[1], f'./Vary_alpha_{para[0]}_eps_{para[6]}',f'control_frame_{epoch}', 'red', ['','$f_\\theta$'])
                        #plot_gif_frame(x, z, 0.5*para[0]*exact[1], f'./Vary_alpha_{para[0]}_eps_{para[6]}',f'adjoint_frame_{epoch}', 'blue', ['','$z_\\theta$'])
        toc = time.time()
        elapseTime = toc - tic
        u, f = generator_NN(x)              
...
repeats = 1
logalpha = 4
#eps_values = list(0.1*np.array(range(1,11))) + list(0.01*np.array(range(5,10)))
eps_values = list(0.01*np.array(range(1,5)))
for eps_value in eps_values:
        alpha = 10**(-logalpha)
        para = para_def(alpha, 0.25*alpha, EpUp = 40, NoUp = 500, h_n = 40, printyn = True, epsilon = eps_value)
        
        path = f'./Vary_alpha_{logalpha}_eps_{para[6]}'
        ...
        exact_z   = np.nan*x
        exact_f   = np.nan*x
        target    = torch.sign(torch.sin(-3*pi*x))
        target[0] = -1
        target[-1]= -1
        ##############################################################################################
        u = 0
