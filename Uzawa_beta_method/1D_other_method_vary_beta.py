...
##############################################################################################
def plot_loss(loss_history, path, filename, para, labels = ['Total', 'Target', 'State', 'Control', 'Adjoint', 'PINNs']):
...
def geo_train(target, para, x, exact = torch.tensor([float('nan')]), learning_rate = 1e-3,  device = torch.device("cpu")):       
        ...
        def generatorCriterion(x, z, target, para):
                ...
                term4 = para[1] * Int1D((uxx + f)**2)

                return term1 + term2_1 + term2_2 + term3 + term4, term1, term2_1, term2_2, term3, term4, u, f, uxx
        ############################################################
        ...
        for epoch in range(para[2]*para[3]):
                ...
                total_loss, Target, Laplace, Control, Adjoint, PINNs, u, f, Au = generatorCriterion(x, z, target, para)                                	
                total_loss.backward()
                generatorOptimiser.step()

                # Append the losses as a list
                loss_history.append([total_loss.item(), Target.item(), Laplace.item(), Control.item(), Adjoint.item(), PINNs.item()])
                if epoch % para[2] == 0:
                        if para[5]:
...
logb_values = range(0,11)
npts = 201
repeats = 1
logalpha = 4
alpha = 10**(-logalpha)
for logbeta in logb_values:
        beta = 10**(-logbeta)
        para = para_def(alpha, beta, EpUp = 40, NoUp = 500)
        
        path = f'./Vary_beta_{logbeta}_nPts_{npts}'
