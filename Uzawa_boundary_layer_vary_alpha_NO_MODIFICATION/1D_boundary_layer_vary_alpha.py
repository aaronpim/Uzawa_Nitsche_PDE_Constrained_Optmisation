...
def geo_train(target, para, x, exact = torch.tensor([float('nan')]), learning_rate = 1e-3,  device = torch.device("cpu")):       
        ...
        def generatorCriterion(x, z, target, para):
                u, f = generator_NN(x, para)  # Unpack both u and f
        ...
        for epoch in range(para[2]*para[3]):
                ...
                total_loss, Target, Control, Adjoint, u, f, Au = generatorCriterion(x, z, target, para)
                total_loss.backward()
                generatorOptimiser.step()

                # Append the losses as a list
                loss_history.append([total_loss.item(), Target.item(), Control.item(), Adjoint.item()])
                if epoch % para[2] == 0:
                        if para[5]:
        ...
        toc = time.time()
        elapseTime = toc - tic
        u, f = generator_NN(x,para)              
        return generator_NN, u, f, err_history, loss_history, elapseTime
##############################################################################################
###### Main Code ######
##############################################################################################
device = torch.device("cpu")
logalpha_values = range(5,10)
npts = 201
repeats = 1
for logalpha in logalpha_values:
        
        alpha = 10**(-logalpha)
        para = para_def(alpha, 0.25*alpha, EpUp = 40, NoUp = 500)
        ...
        ##############################################################################################
        alpha = 10**(-logalpha)
        b = torch.Tensor([(4*alpha)**(-1/4)])
        b = b.to(torch.float64)
        y = x.to(torch.float64)
        middleIndex = int((len(y) + 1)/2)
        A2= torch.sinh(b)/(torch.cos(b)+torch.cosh(b))
        A3= -torch.sin(b)/(torch.cos(b)+torch.cosh(b))
        exact_u = 1 - torch.cosh(y*b)*torch.cos(y*b) + A2*torch.sinh(y*b)*torch.cos(y*b) + A3*torch.cosh(y*b)*torch.sin(y*b)
        exact_f = torch.sinh(y*b)*torch.sin(y*b) - A2*torch.cosh(y*b)*torch.sin(y*b) + A3*torch.sinh(y*b)*torch.cos(y*b)
        for i in range(0,middleIndex):
                exact_u[len(y)-1-i] = exact_u[i]
                exact_f[len(y)-1-i] = exact_f[i]
        exact_f = -2*(b**2)*exact_f
        ...
        mysave(torch.abs(u-exact_u), path, f'State_pointwise')
        mysave(torch.abs(f-exact_f), path, f'Control_pointwise')
