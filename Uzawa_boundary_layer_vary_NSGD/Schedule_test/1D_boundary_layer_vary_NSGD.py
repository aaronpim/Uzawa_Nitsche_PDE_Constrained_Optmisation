        ...
        for epoch in range(para[2]*para[3]):
                ...
                if epoch % para[2] == 0:
                        if para[5]:
                                ...
                                #plot_line(x, u, exact[0], f, exact[1], path,f'State_Control_Epoch_{epoch}')
...
EpUp_values = list(range(50,51)) #+list(range(10,101,30))
for epup in EpUp_values:
        ...
        ##############################################################################################
        alpha = 10**(-logalpha)
        b = torch.Tensor([1/(sqrt(2)*sqrt(sqrt(alpha)))])
        A2= torch.sinh(b)/(torch.cos(b)+torch.cosh(b))
        A3= -torch.sin(b)/(torch.cos(b)+torch.cosh(b))
        exact_u = 1 - torch.cosh(x*b)*torch.cos(x*b) + A2*torch.sinh(x*b)*torch.cos(x*b) + A3*torch.cosh(x*b)*torch.sin(x*b)
        exact_ux  = torch.autograd.grad(exact_u,  x, grad_outputs=torch.ones_like(x), create_graph=True, only_inputs=True)[0]
        exact_f = -torch.autograd.grad(exact_ux, x, grad_outputs=torch.ones_like(x), create_graph=True, only_inputs=True)[0]     
        ...
        for i in range(0,repeats):
                gNN, ui, fi, err_historyi, loss_historyi, elapseTime = geo_train(target, para, x, exact = [exact_u,exact_f], learning_rate = 1e-2) 
                u += ui/repeats
                f += fi/repeats
